package de.eimir.app.notifications
...
import eimir.api.models.NotificationKind
...
fun NotificationsScreen(
    ...
    LazyColumn(
        ...
        items(count = notifications.size, key = { index -> notifications[index].id.toString() }) { index ->
            ...
            val opensSomewhere = de.eimir.app.reference
                .engagementTargetRoute(notification.targetType, notification.targetId) != null
            Surface(
                shape = RoundedCornerShape(EimirTheme.radii.card),
                color = if (unread) EimirTheme.colors.surfaceSubtle else EimirTheme.colors.surface,
                modifier = Modifier
                    .fillMaxWidth()
