...
import de.eimir.app.story.displayNameForUi
import de.eimir.app.shell.PlacePicker
    ...
    ModalBottomSheet(
        ...
        Column(
            modifier = Modifier
                ...
                .padding(bottom = EimirTheme.spacing.step8)
                .imePadding(),
...
private fun SheetHeading(text: String) {
    Text(
        text = text,
        style = MaterialTheme.typography.titleLarge.copy(fontFamily = EimirDisplayFamily),
...
private fun SheetDestructiveAction(labelRes: Int, enabled: Boolean, onClick: () -> Unit) {
    TextButton(
        ...
        Text(stringResource(labelRes), color = EimirTheme.colors.error)
    }
}
