    ...
    LazyColumn(
        ...
        item {
            Surface(
                ...
                Column(modifier = Modifier.padding(EimirTheme.spacing.cardPadding)) {
                    RelatedPersonForm(
                ...
                Row(
                    ...
                    Box {
                        ...
                        IconButton(
                            ...
                            MoreVertGlyph(tint = EimirTheme.colors.textSecondary)
                        }
                        DropdownMenu(
...
private fun DeleteRelatedPersonDialog(
    ...
    AlertDialog(
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(EimirTheme.spacing.step4)) {
                Text(
...
private fun DeleteChoice(
    ...
    Row(
        modifier = Modifier
            ...
            .padding(vertical = EimirTheme.spacing.step2),
    ) {
        RadioButton(selected = selected, onClick = null)
        Column(modifier = Modifier.padding(start = EimirTheme.spacing.step2)) {
            Text(
                text = stringResource(titleRes),
                style = MaterialTheme.typography.bodyLarge,
                // Destructive is set apart by more than colour: it is also the
                // one choice with an explicit warning body beneath it.
                color = if (destructive) EimirTheme.colors.error else EimirTheme.colors.textPrimary,
            )
            Text(
