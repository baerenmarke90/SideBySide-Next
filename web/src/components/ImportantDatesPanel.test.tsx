...
describe("ImportantDatesPanel", () => {
  ...
  beforeEach(() => {
    mockPeopleApi = {
      listImportantDatesApiV1SpacesSpaceIdImportantDatesGet: vi.fn().mockResolvedValue([sampleDate]),
      createImportantDateApiV1SpacesSpaceIdImportantDatesPost: vi.fn().mockImplementation(({ importantDateFields }) =>
        Promise.resolve({
          id: "date-new",
          ...importantDateFields,
          version: 1,
      ...
      updateImportantDateApiV1SpacesSpaceIdImportantDatesDateIdPut: vi.fn().mockImplementation(({ dateId, importantDateFields }) =>
        Promise.resolve({
          id: dateId,
          ...importantDateFields,
          version: 2,
      ...
      deleteImportantDateApiV1SpacesSpaceIdImportantDatesDateIdDelete: vi.fn().mockResolvedValue(undefined),
    } as unknown as PeopleApi;
  });

  function renderPanel(dates = [sampleDate]) {
    mockPeopleApi.listImportantDatesApiV1SpacesSpaceIdImportantDatesGet = vi.fn().mockResolvedValue(dates);
  ...
  it("renders heading Besondere Tage and top add button without manual refresh", async () => {
    ...
    expect(await screen.findByRole("heading", { level: 2, name: /Besondere Tage/i })).toBeTruthy();
    expect(screen.getByRole("button", { name: /\+ Besonderen Tag festhalten/i })).toBeTruthy();
  ...
  it("displays dates in clickable cards without inline edit/delete buttons", async () => {
    ...
    const card = await screen.findByRole("button", { name: /Unser Jahrestag/i });
  ...
  it("opens create modal when clicking + button, dirty checks on cancel", async () => {
    ...
    const addBtn = await screen.findByRole("button", { name: /\+ Besonderen Tag festhalten/i });
    await user.click(addBtn);

    // Dialog title
    expect(screen.getByRole("heading", { level: 2, name: /Besonderen Tag festhalten/i })).toBeTruthy();
    ...
    expect(screen.getByText(/Ungespeicherte Änderungen verwerfen\?/i)).toBeTruthy();

    // Click Weiter bearbeiten
    await user.click(screen.getByRole("button", { name: /Weiter bearbeiten/i }));
    expect(screen.queryByText(/Ungespeicherte Änderungen verwerfen\?/i)).toBeNull();

    // Click cancel again and confirm discard
    await user.click(cancelBtn);
    await user.click(screen.getByRole("button", { name: /Änderungen verwerfen/i }));
  ...
  it("clicking a card opens edit dialog with delete capability inside", async () => {
    ...
    expect(screen.getByRole("heading", { level: 2, name: /Besonderen Tag ändern/i })).toBeTruthy();
    ...
    expect(screen.getByText(/Diesen besonderen Tag wirklich löschen\?/i)).toBeTruthy();
    const confirmDeleteBtn = screen.getByRole("button", { name: /Besonderen Tag löschen/i });
    await user.click(confirmDeleteBtn);

    await waitFor(() => {
      expect(mockPeopleApi.deleteImportantDateApiV1SpacesSpaceIdImportantDatesDateIdDelete).toHaveBeenCalledWith({
        dateId: "date-1",
        spaceId: "space-1",
        ifMatch: "1",
      });
