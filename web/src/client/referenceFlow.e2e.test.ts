...
import { createReferenceApis, runMemoryMediaStoryFlow, signIn } from './referenceFlow';
...
function requiredEnv(name: keyof ImportMetaEnv): string {
  ...
  if (!value) throw new Error(`${String(name)} muss fuer den G2-E2E-Lauf gesetzt sein.`);
  return value;
}
  ...
  e2eTest('runs Memory -> image -> timeline -> authorized read against the real stack', async () => {
    ...
    try {
      ...
      const storyMemory = result.story.items.find(
        (item) => item.kind === 'MEMORY' && item.memory.id === result.memory.id,
      );
