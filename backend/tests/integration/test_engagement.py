...
class TestProjectPendingDiagnostic:
    """#680's own follow-up comment: `project_pending()` persists an
    unexpected projection failure's raw text the same way the Job worker
    used to, into `outbox_events.last_error`. Extended here as the trivial,
    same-primitive reuse the comment asked for; broader Outbox scope stays
    with #695.
    """

    def test_unexpected_projection_failure_persists_only_a_safe_summary(
        self, session: Session, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        canary = "SUPER_SECRET_PRIVATE_TEXT owned by another visitor"
        secret_token = "super-secret-projection-token"

        def failing_projection(_session: Session, _event: OutboxEvent) -> None:
            raise RuntimeError(f"Bearer {secret_token} {canary}")

        monkeypatch.setattr(service, "project_event", failing_projection)

        row = outbox_service.record(session, _projection_event())
        session.flush()

        processed = service.project_pending(session)
    ...
    def test_a_retry_that_succeeds_still_clears_the_failure_diagnostic(
        self, session: Session, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """The existing success contract (`last_error = None`) is unaffected.

        `project_event` itself is stubbed out here: this proves
        `project_pending`'s own success/failure bookkeeping around it, not
        the real projection catalog, which the rest of this file already
        exercises through real domain mutations.
        """
        monkeypatch.setattr(service, "project_event", lambda _session, _event: None)

        row = outbox_service.record(session, _projection_event())
        session.flush()

        outbox_service.mark_failed(row, "temporary canary secret=abc123")
        assert row.last_error is not None
