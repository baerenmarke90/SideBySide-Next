"""Exercise real PostgreSQL races for M3-S8 PrivateCollection ordering."""
...
from eimir.private_collections import service
...
def _setup(production_client):  # type: ignore[no-untyped-def]
    ...
    with maker.begin() as session:
        ...
        partner = AuthorizationContext(ben.id, space.id)
...
def _assert_contiguous(items: list[PrivateCollectionItem]) -> None:
    assert [item.position for item in items] == list(range(len(items)))
    assert len({item.id for item in items}) == len(items)


class TestPrivateCollectionOrderRaces:
    def test_two_parallel_reorders_have_exactly_one_winner(self, production_client) -> None:  # type: ignore[no-untyped-def]
        ...
        def reorder(order: list):  # type: ignore[no-untyped-def]
            with maker.begin() as session:
        ...
        first, second = _concurrently(
            lambda: reorder([world["second_id"], world["first_id"]]),
            lambda: reorder([world["first_id"], world["second_id"]]),
        )
        results = [first, second]
        assert results.count("PRIVATE_COLLECTION_ORDER_CONFLICT") == 1
    ...
    def test_reorder_against_create_is_serialized_by_root_version(self, production_client) -> None:  # type: ignore[no-untyped-def]
        ...
        assert results in (
            {"REORDERED", "CREATED"},
            {"PRIVATE_COLLECTION_ORDER_CONFLICT", "CREATED"},
        )
    ...
    def test_reorder_against_delete_never_leaves_a_position_gap(self, production_client) -> None:  # type: ignore[no-untyped-def]
        ...
        assert results in (
            {"DELETED", "REORDERED"},
            {"DELETED", "PRIVATE_COLLECTION_ORDER_CONFLICT"},
        )
